...
with bmy.select():
    ...
    from mlib.py.test.support.os_helper import TESTFN, FakePath
...
no_chdir = unittest.mock.patch.object(os, "chdir", side_effect=AssertionError("shouldn't call os.chdir()"))
...
class TestRmTree(BaseTest, unittest.TestCase):
    ...
    @unittest.skip("myrrh not fully supported")
...
class TestCopyTree(BaseTest, unittest.TestCase):
    ...
    def test_copytree_retains_permissions(self):
        ...
        self.assertEqual(
            os.stat(os.path.join(src_dir, "permissive.txt")).st_mode,
            os.stat(os.path.join(dst_dir, "permissive.txt")).st_mode,
        )
        self.assertEqual(
            os.stat(os.path.join(src_dir, "restrictive.txt")).st_mode,
    ...
    @unittest.mock.patch.object(os, "chmod")
...
class TestCopy(BaseTest, unittest.TestCase):
    ...
    def test_copystat_symlinks(self):
        ...
        os.utime(src, (src_stat.st_atime, src_stat.st_mtime - 42.0))  # ensure different mtimes
        write_file(dst, "bar")
        ...
        self.assertTrue(abs(os.stat(src).st_mtime - os.stat(dst).st_mtime) < 00000.1)

    @unittest.skipUnless(
        hasattr(os, "chflags") and hasattr(errno, "EOPNOTSUPP") and hasattr(errno, "ENOTSUP"),
    ...
    @unittest.skip("myrrh not supported test")
    ...
    @unittest.skip("myrrh not supported test")
    @unittest.skipIf(
        MACOS or SOLARIS or _winapi,
        "On MACOS, Solaris and Windows the errors are not confusing (though different)",
    )
...
class TestArchives(BaseTest, unittest.TestCase):
    ...
    def test_make_tarball(self):
        ...
        with tarfile.open(tarball, "r:gz") as tf:
            self.assertCountEqual(
                tf.getnames(),
                [".", "./sub", "./sub2", "./file1", "./file2", "./sub/file3"],
            )
    ...
    @unittest.skip("myrrh not supported feature")
    @support.requires_zlib()
...
class TestWhich(BaseTest, unittest.TestCase):
    ...
    def test_environ_path_missing(self):
        with os_helper.EnvironmentVarGuard() as env:
            ...
            # without confstr
            with unittest.mock.patch.object(os, "confstr", side_effect=ValueError, create=True), support.swap_attr(os, "defpath", self.dir):
                rv = shutil.which(self.file)
            self.assertEqual(rv, self.temp_file.name)

            # with confstr
            with unittest.mock.patch.object(os, "confstr", return_value=self.dir, create=True), support.swap_attr(os, "defpath", ""):
                rv = shutil.which(self.file)
            self.assertEqual(rv, self.temp_file.name)
...
class TestMove(BaseTest, unittest.TestCase):
        ...
        try:
            for src, dst in [("srcdir", "srcdir/dest")]:
                ...
                self.assertTrue(
                    ...
                    msg="_destinsrc() wrongly concluded that " "dst (%s) is not in src (%s)" % (dst, src),
                )
        finally:
    ...
    @os_helper.skip_unless_dac_override
    @unittest.skipUnless(
        hasattr(os, "lchflags") and hasattr(stat, "SF_IMMUTABLE") and hasattr(stat, "UF_OPAQUE"),
        "requires lchflags",
    )
...
class TestCopyFileObj(unittest.TestCase):
    ...
    def test_win_impl(self):
        ...
        with unittest.mock.patch.object(shutil, "_copyfileobj_readinto") as m:
