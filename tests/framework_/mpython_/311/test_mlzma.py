...
with bmy.select():
    from mlib.py import _compression

import array

with bmy.select():
    from mlib.py.io import BytesIO, UnsupportedOperation, DEFAULT_BUFFER_SIZE, open
with bmy.select():
...
with bmy.select():
    from mlib.py.test.support import _4G, bigmemtest
    from mlib.py.test.support.import_helper import import_module
    from mlib.py.test.support.os_helper import TESTFN, unlink

lzma = import_module("lzma")
with bmy.select():
    from mlib.py.lzma import LZMACompressor, LZMADecompressor, LZMAError, LZMAFile
...
class CompressorDecompressorTestCase(unittest.TestCase):
    ...
    def test_decompressor_unused_data(self):
        lzd = LZMADecompressor()
        extra = b"fooblibar"
        self._test_decompressor(lzd, COMPRESSED_XZ + extra, lzma.CHECK_CRC64, unused_data=extra)
