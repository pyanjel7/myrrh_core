    ...
    if not os.path.exists(index) or force_build:
        ...
        try:
            ...
            if hasattr(mod, "__doc__"):
                ...
                o.__doc__ = mod.__doc__() if callable(mod.__doc__) else mod.__doc__  # type: ignore[misc,operator]
                o.__doc__ = o.__doc__ and o.__doc__.replace("_main_", alias_or_module_name) or None
        ...
        html_index = pdoc.render.html_index(docs)
        if html_index:
            (tmpdir / "index.html").write_bytes(html_index.encode())

        search_index = pdoc.render.search_index(docs)
        if search_index:
            (tmpdir / "search.js").write_bytes(search_index.encode())
