...
from ...interfaces import IMyrrhOs, IRuntimeObject, EWiring
...
from ....provider import EProtocol as _Protocol, EWiring as _Wiring
...
class RuntimeSyscall:
    ...
    def _init_objects(self):
        self.objects = RuntimeObjectManager(self.myrrh_os, self.MAX_FDS)

        self.hin = self.objects.append(
            InputIo(),
            FileStream(sys.stdout.fileno(), "", "stdout", getattr(Entity, ".").system.stream),
            FileStream(sys.stderr.fileno(), "", "stderr", getattr(Entity, ".").system.stream),
        )
        self.hout = self.hin + 1
        self.herr = self.hin + 2

    def _init_tasks(self):
        self.tasks = RuntimeTaskManager(self.myrrh_os, self.POOL_SIZE, self.myrrh_os.reg.eid)
    ...
    def stream_in(
        ...
        *,
        ...
        fd = self.open_file(file_path, wiring=EWiring.IN, extras=extras, protocol=protocol)
    ...
    def stream_out(
        ...
        *,
        ...
        fd = self.open_file(
            file_path,
            wiring=EWiring.OUT | EWiring.CREATE | EWiring.RESET,
            extras=extras,
            protocol=protocol,
        )
        chunk_size = self.CHUNK_SIZE
